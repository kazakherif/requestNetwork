...
export type TheGraphClient<
  TGraphClientVariant extends 'near' | undefined
...
export const getTheGraphClient = <TGraphClientVariant extends 'near' | undefined = undefined>(
  network: string,
  options?: TheGraphClientOptions,
): TheGraphClient<TGraphClientVariant> => {
  ...
  if (network === 'aurora') {
    return getSdkNear(new GraphQLClient(url, options)) as TheGraphClient<'near'>;
  } else {
    return getSdk(new GraphQLClient(url, options)) as TheGraphClient<undefined>;
  }
};
